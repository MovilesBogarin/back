...
exports.deleteRecipe = asyncHandler(async (req, res) => {
    try {
        ...
        const ids = schedule_recipes.filter(schedule_recipe => schedule_recipe.id_recipe === parseInt(id)).map(schedule_recipe => schedule_recipe.id_schedule);
        ids.forEach(id => {
            const index = schedule_recipes.findIndex(schedule_recipe => schedule_recipe.id_schedule === id);
